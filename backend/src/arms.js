...
const SALES_LOOKBACK_DAYS =
  Number(
    process.env.ARMS_SALES_LOOKBACK_DAYS || 30
  );
...
function cleanCustomerName(value) {
  ...
  // Remove ARMS placeholders.
  if (/^(?:arms\s+customer|customer|member)$/i.test(name)) {
    return null;
  }

  // ARMS/Hasani names commonly appear as:
  // Mrs KB : NUR ZULAIKHA NABILA...
  // Mr KG: ABC...
  // Remove the requested title/category prefix only.
  name = name
...
function extractMemberName(
  ...
  // Prefer the more specific fields before the generic Name field.
  const labels = [
    "Full Name",
    "Customer Name",
    "Member Name",
  ...
  for (const label of labels) {
    const value =
      extractTableValue(html, label) ||
      extractFieldFromText(html, label);
  ...
  // Last fallback: look for a labelled full-name value in raw text.
  ...
  for (const pattern of patterns) {
    ...
    const cleaned = cleanCustomerName(match && match[1]);
    if (cleaned) {
      return cleaned;
    }
  }
...
function extractIssueBranch(
  ...
  return (
    extractTableValue(html, "Issue Branch") ||
    extractTableValue(html, "Issuing Branch") ||
    extractTableValue(html, "Apply Branch") ||
    extractTableValue(html, "Apply branch") ||
    extractFieldFromText(html, "Issue Branch") ||
    extractFieldFromText(html, "Issuing Branch") ||
    extractFieldFromText(html, "Apply Branch") ||
    null
  );
}
...
/* ==========================================================================
   COMPLETE MEMBER PARTICULARS
   ========================================================================== */

function extractMemberParticular(html, labels) {
  const candidates = Array.isArray(labels)
    ? labels
    : [labels];
...
function extractMemberNRIC(html) {
  return extractMemberParticular(html, [
    "NRIC / Passport No.",
    "NRIC/Passport No.",
    "NRIC / Passport",
    "NRIC",
...
function extractMemberFullName(html) {
  return (
    extractMemberName(html) ||
    cleanCustomerName(
      extractMemberParticular(html, [
        "Full Name",
        "Customer Name",
        "Member Name",
        "Name"
...
function extractMemberMembershipType(html) {
  return (
    extractMemberParticular(html, [
      "Membership Type",
      "Member Type"
    ]) ||
    extractMemberType(html) ||
    null
  );
}


function extractMemberTitle(html) {
  return extractMemberParticular(html, [
    "Title"
  ]);
}
...
function extractMemberRace(html) {
  return extractMemberParticular(html, [
    "Race"
  ]);
}
...
function extractMemberNationality(html) {
  return extractMemberParticular(html, [
    "Nationality",
    "National"
  ]);
...
function extractMemberApplyBranch(html) {
  return extractMemberParticular(html, [
    "Apply Branch",
    "Apply branch"
  ]);
...
function extractMemberLastRenewBranch(html) {
  return extractMemberParticular(html, [
    "Last Renew Branch",
    "Last Renewal Branch"
  ]);
...
function extractMemberLastPurchaseBranch(html) {
  return extractMemberParticular(html, [
...
function extractMemberTerminatedDate(html) {
  return extractMemberParticular(html, [
    "Terminated Date"
  ]);
}
...
function extractMemberBlockedDate(html) {
  return extractMemberParticular(html, [
    "Blocked Date"
  ]);
}
...
function extractMemberVerifiedBy(html) {
  return extractMemberParticular(html, [
    "Verified By"
  ]);
}
...
function extractPersonalInformation(html) {
  const fullName =
    extractMemberFullName(html);

  const memberType =
    extractMemberMembershipType(html);
  ...
  return {
    cardNo:
      extractMembershipNumber(html),

    nric:
      extractMemberNRIC(html),
    ...
    race:
      extractMemberRace(html),

    nationality:
      extractMemberNationality(html),

    applyBranch:
      extractMemberApplyBranch(html),

    issueBranch:
      extractIssueBranch(html),

    issueDate:
      extractIssueDate(html),

    expiryDate:
      extractExpiryDate(html),

    lastRenewBranch:
      extractMemberLastRenewBranch(html),

    lastPurchaseBranch:
      extractMemberLastPurchaseBranch(html),

    terminatedDate:
      extractMemberTerminatedDate(html),

    blockedDate:
      extractMemberBlockedDate(html),

    verifiedBy:
      extractMemberVerifiedBy(html),

    points:
      extractPoints(html),

    pointsUpdate:
      extractPointsUpdate(html)
  };
}
...
function extractSalesDetailRows(
  ...
  while (
     ...
     * 6 Over
     *
     * Some ARMS versions also keep the original
     * trans_detail(...) JavaScript call inside the row.
     * Preserve it when available so the customer portal
     * can fetch the real item-level receipt later.
     */
